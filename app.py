...
# --- Pasta Download dentro do projeto ---
...
def ensure_playwright_browsers(engine: str):
    """No Streamlit Cloud, evitamos chamar `playwright install` para não disparar sudo.
    Confiamos na instalação padrão de browsers do Playwright.
    """
    return
    ...
    def processar_chave(self, chave: str) -> Dict:
        try:
            ...
            return {
                "chave": chave,
                "status": "falha",
                "detalhe": f"Erro ao abrir site: {e}",
        ...
        for sel in [s.strip() for s in settings.botao_baixar_xml.split(",") if s.strip()]:
            try:
                ...
                # --- NOVO: salvar diretamente na pasta Download do projeto ---
                dest_path = DOWNLOAD_DIR / suggested
                download.save_as(dest_path)

                # Lê o arquivo para disponibilizar o botão de download no Streamlit
...
if not sys.platform.startswith("win"):
    # Em ambientes Linux (como Streamlit Cloud), força headless para evitar problemas de display.
    headless = True
...
if st.session_state.get("resultados"):
    ...
    for r in st.session_state.resultados:
        ...
        if r.get("status") == "ok":
            base["arquivo"] = r.get("filename")
        else:
            base["detalhe"] = r.get("detalhe")
        linhas.append(base)
